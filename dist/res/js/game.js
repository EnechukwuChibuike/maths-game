...
if (gameStatus && gameStatus != 0 && gameStatus != null) {
	...
	document.getElementById('selectedOperator').innerHTML =
		(gameMathOperator == 1 ? "Addition (+)" : 
			(gameMathOperator == 2 ? "Subtraction (-)" : 
				(gameMathOperator == 3 ? "Division (/)" : 
				(gameMathOperator == 4 ? "Multiplication (*)" : "Invalid Operator"))))
...
function newQuestion(gameMathOperator) {
 	...
 	let operator = (gameMathOperator == 1 ? "+" : 
			(gameMathOperator == 2 ? "-" : 
				(gameMathOperator == 3 ? "/" : 
				(gameMathOperator == 4 ? "*" : "Invalid Operator"))))
...
function submit() {
	let num1 = parseInt(document.getElementById('num1').innerHTML)
 	...
 	if (userAnswer == correctAnswer) {
 		...
 		document.getElementById('currentPoints').innerHTML = gameMathPoint		
 		if (gameMathPoint > 0) {
 			alert("Wrong Answer, The Correct Answer is " +correctAnswer + " !!! You just lost a point")
 			newQuestion(newQuestionOperator)
 		}else{
 			alert("Wrong Answer, The Correct Answer is " +correctAnswer + " !!! Game Over You have no Points Left")
